#date: 2022-04-01T17:11:16Z
#url: https://api.github.com/gists/1f980ed333432f850ea038a7fff7d95f
#owner: https://api.github.com/users/crapthecoder
...
SNAKE_SIZE = gcd(WINDOW_SIZE[0], WINDOW_SIZE[1]) // 4
SNAKE_SPEED = SNAKE_SIZE // 12
WIDTH = SNAKE_SIZE // 10
MARGIN = SNAKE_SIZE * 4
...
FONT_SIZE = SNAKE_SIZE * 2

if SNAKE_SIZE % SNAKE_SPEED != 0:
    raise Exception('SNAKE_SIZE should be divisible by SNAKE_SPEED')
...
class Food:
    ...
    def create_food(self):
        ...
        self.coords = random.choice(
            [(i, j) for i in range(0, WINDOW_SIZE[0], SNAKE_SIZE) for j in range(MARGIN, WINDOW_SIZE[1], SNAKE_SIZE) if
             (i, j) not in check])
...
class Snake:
    ...
    def move(self):
        if self.last_key_function != -1:
            for part_index, (part_coords, part_velocity) in enumerate(
                    self.body if not self.lock_last else self.body[:-1]):
...
class Game:
    def __init__(self):
        ...
        self.init_head = (WINDOW_SIZE[0] // 2 // SNAKE_SIZE * SNAKE_SIZE,
                          WINDOW_SIZE[1] // 2 // SNAKE_SIZE * SNAKE_SIZE)
        ...
        while not self.finished:
            ...
            pygame.draw.rect(self.canvas, WHITE, ((0, MARGIN - WIDTH), (WINDOW_SIZE[0], WIDTH)))
