#date: 2025-02-21T17:09:03Z
#url: https://api.github.com/gists/3691873fa93dc6f3e1960f30f1ff8f43
#owner: https://api.github.com/users/Chugeno
...
def inference_video(args, video_save_path, device=None, total_workers=1, worker_idx=0):
    # Agregar detección de MPS
    if device is None:
        print("Verificando dispositivo disponible...")
        if torch.backends.mps.is_available():
            print("MPS está disponible - usando GPU M1")
    ...
    while True:
        ...
        # Modificar la sincronización según el dispositivo
        if device is not None:
            if device.type == 'cuda':
                torch.cuda.synchronize(device)
            elif device.type == 'mps':
                torch.mps.synchronize()  # Sincronización para MPS
        pbar.update(1)
...
def run(args):
    ...
    # Modificar esta parte para Mac M1
    if torch.backends.mps.is_available():
        print("Usando MPS (GPU M1)")
        num_process = 1  # Para M1, usamos un solo proceso
    else:
