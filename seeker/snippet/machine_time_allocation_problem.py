#date: 2023-07-03T16:41:43Z
#url: https://api.github.com/gists/fa88e9bc46b4c1e3960e6209e4c0ea36
...
problem += lpSum([task_priorities[task] * allocation[task, machine] for task in tasks for machine in machines_available]), "Total Priority"
...
# Print the optimal allocation of machine time and their priorities
for task in tasks:
    for machine in machines_available:
        if allocation[task, machine].varValue == 1:
            print(f"Task '{task}' allocated to {machine} with priority {task_priorities[task]}")

# Print the total priority achieved
print("Total Priority:", problem.objective.value())
