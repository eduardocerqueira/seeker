#date: 2022-11-03T17:00:29Z
#url: https://api.github.com/gists/4ee494d612527e98e59299e4ee406b9b
#owner: https://api.github.com/users/malbertosp3
    ...
    for key in sorted(entidade):
        ...
        if type(membro) is dict:
            ...
            if key == "codigoCargo" and f"{valor_membro(membro)}" != "('cargoConstitucional', 'presidente')":
                break
            if key == "municipio":
                print(f"{state},{valor_membro(membro)}", end='')
...
def main():
    ...
    parser.add_argument("-s", "--state", type=str, required=True,
                        help="Sigla do estado (ex: Acre = ac)")
    ...
    f = []
    for (dirpath, dirnames, filenames) in walk("./urna/" + state_path):
        f.extend(filenames)
        break
    
    print(f"estado,municipio,zona,secao,candidato_13,candidato_22")
