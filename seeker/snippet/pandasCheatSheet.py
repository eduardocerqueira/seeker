#date: 2025-08-08T16:51:59Z
#url: https://api.github.com/gists/14398378e34f964b8e2947cddb97b6b0
...
# group with custom aggregations
df.groupby(['gsm','sex']).agg({'page_id': 'count', 'appearances': ['mean','sum'], 'year': ['mean','std']})

# Get top n for each group of columns in a sorted dataframe
# (make sure dataframe is sorted first)
top5 = df.groupby(['groupingcol1', 'groupingcol2']).head(5)

# Convert multiindexed group by result into multi-column index
# https://stackoverflow.com/questions/10373660/converting-a-pandas-groupby-object-to-dataframe
gb = df.groupby(['media','id','type','action']).agg({'action': 'count', 'changeDuration': 'sum', 'changeVelocity': 'sum'})
gb = gb.add_suffix('_Agg').reset_index()
