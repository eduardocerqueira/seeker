#date: 2021-12-01T16:58:19Z
#url: https://api.github.com/gists/c6ba57f22f4eefd28770a7407efb4f62
#owner: https://api.github.com/users/timhawes
...
def _encode(input, len):
    k = len >> 2
    return struct.pack(*("%iI" % k,) + tuple(input[:k]))

def _decode(input, len):
    k = len >> 2
    return struct.unpack("%iI" % k, input[:len])
...
def test(input=""):
    ...
    import hashlib
    print((repr(hashlib.md5(input).hexdigest())))
