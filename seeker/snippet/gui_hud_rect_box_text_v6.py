#date: 2025-08-13T16:56:46Z
#url: https://api.github.com/gists/ddf7f21e45e61ae2e773e2f485be84ed
...
class GLTex:
    def __init__(self,w,h,surf=None):
        ...
        else:glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,self.w,self.h,0,GL_RGBA,GL_UNSIGNED_BYTE,pygame.image.tostring(surf,'RGBA',True))
...
class Message:
    def __init__(self, text, pos, font, fg, bg, fade_in, vis, fade_out, fade_speed=1.0, fade_speed_in=None, fade_speed_out=None, box_op=1.0, blur_r=6, padding=PADDING, pad_top=None, pad_bottom=None, pad_left=None, pad_right=None, max_w=None, shared_blur=None):
        self.text=text;self.x,self.y=pos;self.font=font;self.fg=fg;self.bg=bg
        self.fade_in, self.visible, self.fade_out = map(float, (fade_in, vis, fade_out))
        # Fade speed multipliers: >1 = faster (shorter duration), <1 = slower
        self.fade_speed = float(fade_speed)
        self.fade_speed_in = float(fade_speed_in) if fade_speed_in is not None else None
        self.fade_speed_out = float(fade_speed_out) if fade_speed_out is not None else None
        fsi = self.fade_speed_in if self.fade_speed_in is not None else self.fade_speed
        fso = self.fade_speed_out if self.fade_speed_out is not None else self.fade_speed
        if fsi <= 0: fsi = 1.0
        if fso <= 0: fso = 1.0
        self.eff_fade_in = float(self.fade_in) / float(fsi)
        self.eff_fade_out = float(self.fade_out) / float(fso)
        self.box_op = float(box_op); self.blur = int(blur_r)
    ...
    def update_and_draw(self,now,draw_prog,sw,sh):
        e = now - self.start
        total = self.eff_fade_in + self.visible + self.eff_fade_out
        if e >= total:
            self.finished = True
            return
        if e < self.eff_fade_in:
            fa = e / self.eff_fade_in
        elif e < self.eff_fade_in + self.visible:
            fa = 1.0
        else:
            fa = 1.0 - ((e - self.eff_fade_in - self.visible) / self.eff_fade_out)
...
def make_spawner(msgs, base_font, blur_prog):
    ...
    def spawn_message(text, pos=(20,20), fg=(255,255,255), bg=(30,30,30),
                      ...
                      fade_speed=1.0, fade_speed_in=None, fade_speed_out=None,
        ...
        fade_speed: overall multiplier for fade durations (>1.0 = faster, <1.0 = slower).
        fade_speed_in / fade_speed_out: optional per-direction overrides.
        """
        ...
        if font is None:
            if font_path is not None:
                ...
                font = pygame.font.SysFont(None, size)
        m = Message(text, pos, font, fg, bg, fade_in, visible, fade_out, fade_speed, fade_speed_in, fade_speed_out, box_opacity, blur_radius, padding, pad_top, pad_bottom, pad_left, pad_right, max_width, blur_prog)
        msgs.append(m)
        return m
