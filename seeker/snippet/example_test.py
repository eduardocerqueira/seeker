#date: 2023-06-20T16:54:01Z
#url: https://api.github.com/gists/4d9de5598c24405e52d1bdbf002f7c1c
#owner: https://api.github.com/users/mayrf
...
class ExampleTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Starting Chaincode Labs test")
        """
        Try getting node 1 to mine another block, 
        send it to node 2, and check that node 2 received it. 
        """
        self.log.info("Disconnect node 1 and 2")
        self.disconnect_nodes(1, 2)

        self.log.info("Generate a new block on node 1")
        new_block_hash = self.nodes[1].generate(nblocks=1, invalid_call=False)[0]
        new_block_height = self.nodes[1].getblockcount()

        self.log.info("Check that new block exists on node 1 but not on node 2")
        assert (self.nodes[2].getbestblockhash() != new_block_hash)
        assert_equal(self.nodes[1].getbestblockhash(), new_block_hash)
        assert_equal(new_block_height - 1, self.nodes[2].getblockcount())

        self.log.info("Connect nodes 1 and 2")
        self.connect_nodes(1,2)

        self.log.info("Check that new block exists on node 2")
        assert_equal(self.nodes[2].getbestblockhash(), new_block_hash)
        assert_equal(self.nodes[2].getblockcount(), new_block_height)
