...
def _read_backend_module_sources() -> dict[str, str]:
    if EMBEDDED_BACKEND_MODULES:
        return dict(EMBEDDED_BACKEND_MODULES)
    backend_root = script_dir()
    return {
        name: (backend_root / name).read_text(encoding='utf-8')
        for name in BACKEND_MODULE_FILES
    }
...
def _build_standalone_source() -> str:
    ...
    if marker not in source:
        raise RuntimeError('Unable to build standalone clawssh.py: payload marker not found.')
    payload = repr(_read_backend_module_sources())
    return source.replace(marker, f'EMBEDDED_BACKEND_MODULES = {payload}', 1)


def ensure_embedded_backend_modules() -> None:
    if not EMBEDDED_BACKEND_MODULES:
        return
    runtime_dir = state_dir() / 'backend-runtime'
    runtime_dir.mkdir(parents=True, exist_ok=True)
    os.chmod(runtime_dir, 0o700)
    for module_name, module_source in EMBEDDED_BACKEND_MODULES.items():
        target = runtime_dir / module_name
        if not target.exists() or target.read_text(encoding='utf-8') != module_source:
...
def import_backend_module(name: str):
    ensure_embedded_backend_modules()
...
def auth_cli_path() -> Path:
    ensure_embedded_backend_modules()
    if EMBEDDED_BACKEND_MODULES:
        return state_dir() / 'backend-runtime' / 'auth.py'
    return script_dir() / 'auth.py'
...
def print_invite_qr(invite_link: str, output_png: Path | None = None) -> None:
    if not invite_link:
        return
    require_qrencode()
    print('Scan this QR code in the ClawSSH app:')
    subprocess.run(
        ['qrencode', '-t', 'ASCII', '-s', '1', '-m', '1', '-l', 'L', invite_link],
        check=False,
    )
    if output_png:
        result = subprocess.run(
        ...
        if result.returncode == 0:
            ...
            print(f'Saved smaller, shareable QR code to: {output_png}')
        else:
...
def enroll_device(device_id: str, bind_host: str, connect_host: str) -> None:
    ...
    # Optimization: Strip PEM headers and newlines to save ~100 bytes
    ca_pem = ca_cert_file().read_text(encoding='utf-8')
    ca_base64 = ''.join(
        line.strip()
    ...
    invite_payload = {
        ...
        'p': ca_base64, # caCertificatePem (compact)
    }
    if device_id:
