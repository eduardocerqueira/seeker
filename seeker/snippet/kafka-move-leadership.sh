#date: 2024-09-18T16:49:56Z
#url: https://api.github.com/gists/97732ef53ee3c2f6ea421db8d22f85a2
#owner: https://api.github.com/users/agrezende
...
# Requirements
# ============
# - Kafka 0.8.1.1 (later versions may work, too)
#
#
# Usage
# =====
...
# -------
#
# Step 1 (generate reassignment JSON):
#
#    $ kafka-move-leadership.sh --broker-id 4 --first-broker-id 0 --last-broker-id 8 --zookeeper zookeeper1:2181 > partitions-to-move-4.json
#
# Step 2 (start reassignment process):
#
#    $ kafka-reassign-partitions.sh --zookeeper zookeeper1:2181 --reassignment-json-file partitions-to-move-4 --execute
#
# Step 3 (monitor progress of reassignment process):
#
#    $ kafka-reassign-partitions.sh --zookeeper zookeeper1:2181 --reassignment-json-file partitions-to-move-4 --verify
...
print_usage() {
  ...
  echo "-------"
  echo
  echo "The following example moves leadership from broker with ID 4 to brokers randomly selected from"
  echo "the ID range 0,1,2,3,4,5,6,7,8 (though 4 itself will be excluded from the range automatically):"
  echo
...
declare -r KAFKA_TOPICS_SCRIPT_NAME="kafka-topics.sh"
declare -r FALLBACK_PATH="/opt/kafka/bin"

which "$KAFKA_TOPICS_SCRIPT_NAME" &>/dev/null
if [ $? -ne 0 ]; then
  declare -r FALLBACK_BIN="$FALLBACK_PATH/$KAFKA_TOPICS_SCRIPT_NAME"
  which "$FALLBACK_BIN" &>/dev/null
  if [ $? -ne 0 ]; then
    echo "ERROR: $KAFKA_TOPICS_SCRIPT_NAME (ships with Kafka) not found in PATH."
    exit 70
  else
    declare -r KAFKA_TOPICS_BIN="$FALLBACK_BIN"
  fi
else
  declare -r KAFKA_TOPICS_BIN="$KAFKA_TOPICS_SCRIPT_NAME"
fi
...
function replace_broker {
  ...
  local replacement_broker=`other_broker $brokers_string $broker`
