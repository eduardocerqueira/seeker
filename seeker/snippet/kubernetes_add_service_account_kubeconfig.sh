#date: 2023-03-13T17:05:54Z
#url: https://api.github.com/gists/ffc5fed82b6128a7549fd6d5696d98a8
...
create_service_account_secret() {
    echo -e "\\nCreating a service account secret in ${NAMESPACE} namespace: "**********"
    kubectl apply -f - <<EOF
    apiVersion: v1
    kind: "**********"
    metadata:
...
get_secret_name_from_service_account_legacy() {
    ...
    SECRET_NAME= "**********"="${NAMESPACE}" -o json | jq -r .secrets[].name)
    echo "Secret name: "**********"
}
