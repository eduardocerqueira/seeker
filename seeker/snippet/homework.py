#date: 2024-07-08T17:08:03Z
#url: https://api.github.com/gists/05bb55a59361ae46d0eca642f0a272b6
...
# Return -1 # if no peak element is found.
    ...
    def find_peak_element(arr):
        ...
        # Edge case: If array has fewer than 3 elements, return -1
        if n < 3:
            return -1
...
def delete_duplicates(arr):
    ...
    # Traverse the list from the second element onwards
    for i in range(1, len(arr)):
        # If current element is different from previous element
        if arr[i] != arr[i - 1]:
            arr[write_index] = arr[i]  # Move the unique element to write_index
            write_index += 1  # Move write_index forward

    # Fill remaining elements with zeroes
    for j in range(write_index, len(arr)):
        arr[j] = 0

    # Return modified list and count of unique elements
    return arr, write_index



input_list = [1, 2, 2, 3, 4, 4, 4, 5]
result_list, unique_count = delete_duplicates(input_list)
print(f"Output: {result_list}, {unique_count} (unique values)")
