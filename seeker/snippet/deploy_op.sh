#date: 2023-02-13T16:55:29Z
#url: https://api.github.com/gists/e3eb0d2740b4cb11d55d32ecbd057879
#owner: https://api.github.com/users/keroro520
...
export L1_ENDPOINT="https://data-seed-prebsc-1-s1.binance.org:8545"

# ethereum-optimism/optimism 仓库的根目录
export OP_ROOT_DIR=${OP_ROOT_DIR?"required"}
export OP_NETWORK_NAME="bsctestnet"
...
output_addresses() {
    ...
    SDK_ADDRESSES_JSON_PATH=$OP_DATA_DIR/sdk_addresses.json
    ...
    sdk_addresses="{
        ...
        \"L1CrossDomainMessenger\":     $(echo $addresses | jq '.Proxy__OVM_L1StandardBridge'),
        \"L1StandardBridge\":           $(echo $addresses | jq '.OptimismPortalProxy'),
...
launch_op_geth() {
    docker run \
        ...
        -v "$OP_ROOT_DIR/ops-bedrock/test-jwt-secret.txt: "**********"
        l2geth: "**********"
    sleep 30
}
...
launch_op_batcher() {
    docker run \
        ...
        --env OP_BATCHER_POLL_INTERVAL="1s" \
