...
do
    if [[ -f ./System/Library/LaunchAgents/${agent}.plist.bak && ! -f ./System/Library/LaunchAgents/${agent}.plist ]] ; then
        ...
        echo "[FAIL] Agent ${agent}.plist.bak didn't exist or has a .plist file already"
    fi
done
...
do
    if [[ -f ./System/Library/LaunchDaemons/${daemon}.plist.bak && ! -f ./System/Library/LaunchDaemons/${daemon}.plist ]] ; then
        ...
        echo "[FAIL] Daemon ${daemon}.plist.bak didn't exist or has a .plist file already"
    fi
done
mount -ur / || true
echo "DONE"
