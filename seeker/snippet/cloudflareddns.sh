#date: 2024-11-29T17:12:11Z
#url: https://api.github.com/gists/ea109154306714ecdc3937185c6aa8cb
#owner: https://api.github.com/users/Hukeqing
...
res=$(curl -s -X GET "$listDnsApi" -H "Authorization: "**********":application/json")
resSuccess=$(echo "$res" | jq -r ".success")


if [[ $ipv6 = "true" ]]; then ## Adding new commands, if Synology didn't provided IPv6
resv6=$(curl -s -X GET "$listDnsv6Api" -H "Authorization: "**********":application/json");
fi
...
if [[ $recordId = "null" ]]; then
    ...
    res=$(curl -s -X POST "$createDnsApi" -H "Authorization: "**********":application/json" --data "{\"type\":\"$recordType\",\"name\":\"$hostname\",\"content\":\"$ipAddr\",\"proxied\":$proxy}")
else
    # Record exists
    res=$(curl -s -X PUT "$updateDnsApi" -H "Authorization: "**********":application/json" --data "{\"type\":\"$recordType\",\"name\":\"$hostname\",\"content\":\"$ipAddr\",\"proxied\":$recordProx}")
fi
if [[ $ipv6 = "true" ]] ; then
	if [[ $recordIdv6 = "null" ]]; then
    # IPv6 Record not exists
	proxy="true"; # new entry, enable proxy by default
    res6=$(curl -s -X POST "$createDnsApi" -H "Authorization: "**********":application/json" --data "{\"type\":\"$recType6\",\"name\":\"$hostname\",\"content\":\"$ip6Addr\",\"proxied\":$proxy}");
	else
    # IPv6 Record exists
    res6=$(curl -s -X PUT "$update6DnsApi" -H "Authorization: "**********":application/json" --data "{\"type\":\"$recType6\",\"name\":\"$hostname\",\"content\":\"$ip6Addr\",\"proxied\":$recordProxv6}");
	fi;
