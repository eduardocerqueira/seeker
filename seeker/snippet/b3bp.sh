...
# Copyright (c) 2023 Angel Riera and contributors
...
# Uncomment following line if need, for ex., enviroment variables
...
__dir="${__dir:-$(cd "$(dirname -- "$(readlink -f "${BASH_SOURCE[${__b3bp_tmp_source_idx:-0}]}")")" && pwd)}"
__file="${__dir}/$(basename -- "$(readlink -f "${BASH_SOURCE[${__b3bp_tmp_source_idx:-0}]}")")"
  ...
  while getopts "${__b3bp_tmp_opts}" __b3bp_tmp_opt; do
    ...
    if [[ "${!__b3bp_tmp_varname}" != "0" ]]; then
      ...
      if [[ -z "${OPTARG}" ]]; then
        ...
        __b3bp_tmp_value=$((${!__b3bp_tmp_varname}))
