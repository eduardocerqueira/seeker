#date: 2026-02-10T17:48:46Z
#url: https://api.github.com/gists/ce65a9ebecb6636cf77ce8f7a0d7be58
...
def inner_steps(model, data_iterator, optimizer, num_steps, device):
    ...
    """
    next_batch = data_iterator.__next__
    run_model = model
    ...
    for step in range(num_steps):
        ...
        batch = next_batch().to(device=device, dtype=torch.long, non_blocking=True)

        # Prepare inputs and labels (causal LM: "**********"
        # input_ids: "**********": all tokens except first
        input_ids = batch[:, :-1]
        labels = batch[:, 1:]
        ...
        opt_step()
        opt_zero_grad(set_to_none=True)

        # Track metrics
        total_tokens += "**********"
        if step == last_step:
            # Keep logits from only the last step for verification.
            # Avoid per-step dtype conversion/copies in the hot loop.
            final_logits = logits.detach()
            # Read loss only once to avoid unnecessary device syncs.
...
if __name__ == "__main__":
    ...
    for i in range(num_evals):
        ...
        print(
            f"  Eval {i + 1}: "**********":.2f}s, tokens={result.total_tokens:,}, loss={result.final_loss:.4f}"
        )

    print()
    print("Done!")ns:,}, loss={result.final_loss:.4f}"
        )
