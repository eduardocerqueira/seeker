...
5) Do not emit the token {completion_token} unless ALL stories in prd.json are passes= "**********"
...
def main() -> int:
    ...
    ap.add_argument("--completion-token", type= "**********"=DEFAULT_COMPLETION)
    ...
    for i in range(1, args.max_iterations + 1):
        ...
        if story is None:
            ...
            append_progress(
                args.progress,
                f"[{now_iso()}] ALL_PASS + FINAL_TESTS_PASS + CLEAN = "**********"
            )
            log("All completion conditions met")
            print(args.completion_token, flush= "**********"
