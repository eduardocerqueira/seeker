//date: 2024-01-12T16:54:37Z
//url: https://api.github.com/gists/74fe8d58cd9c4cef73dc4b1b17a7ef1f
//owner: https://api.github.com/users/hadjri
...
func FinalizePatch(ctx context.Context, p *patch.Patch, requester string, githubOauthToken string) (*Version, error) {
 ...
 "**********"	 "**********"i "**********"f "**********"  "**********"g "**********"i "**********"t "**********"h "**********"u "**********"b "**********"O "**********"a "**********"u "**********"t "**********"h "**********"T "**********"o "**********"k "**********"e "**********"n "**********"  "**********"= "**********"= "**********"  "**********"" "**********"" "**********"  "**********"{ "**********"
		if err != nil {
			return nil, err
		}
		githubOauthToken, err = "**********"
		if err != nil {
			return nil, err
		}
	}
...
func getFullPatchParams(p *patch.Patch) ([]patch.Parameter, error) {
	...
	if p.PAlias == "" || !IsPatchAlias(p.PAlias) {
		return p.Parameters, nil
	}
	aliases, err := findAliasesForPatch(p.Project, p.PAlias, p)
	if err != nil {
		return nil, errors.Wrapf(err, "retrieving alias '%s' for patch '%s'", p.PAlias, p.Id.Hex())
	}
...
func MakeMergePatchFromExisting(ctx context.Context, settings *evergreen.Settings, existingPatch *patch.Patch, commitMessage string) (*patch.Patch, error) {
	...
	patchDoc := &patch.Patch{
		...
		Status:               evergreen.VersionCreated,
		PAlias:               evergreen.CommitQueueAlias,
	...
	project.BuildProjectTVPairs(patchDoc, patchDoc.PAlias)
