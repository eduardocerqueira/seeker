...
def test_sendmail_draft_a(driver,param):
...
def test_sendmail_draft_b(driver,param):
    allure.dynamic.title("存草稿箱-编辑过程中主动触发存草稿")
    initialize(driver,driver.current_url)
    WebDriverWait(driver, 10,0.1).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR,".title"))).click()
    WebDriverWait(driver, 10,0.1).until(
    ...
    #存完草稿可继续编辑写信页面，重写正文
    result2=send_text(driver,txt=param["send_text"])
