#date: 2025-07-28T17:02:03Z
#url: https://api.github.com/gists/ab1b9d69630b91b01604a8ce1369e910
#owner: https://api.github.com/users/jamesagarside
...
log stream --predicate 'subsystem == "com.apple.cmio" and composedMessage contains "streaming for client"' | while read line; do
  ...
  if echo "$line" | grep "start"; then
	...
	curl --location --request PUT 'http://192.168.3.204:9123/elgato/lights' --header 'Content-Type: application/json' --data-raw '{"lights":[{"brightness":40,"temperature":162,"on":1}],"numberOfLights":1}'
  ...
  if echo "$line" | grep "stop"; then
	...
	#Light 1 - set to off
	curl --location --request PUT 'http://192.168.3.204:9123/elgato/lights' --header 'Content-Type: application/json' --data-raw '{"lights":[{"brightness":40,"temperature":162,"on":0}],"numberOfLights":2}'
