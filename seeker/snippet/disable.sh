#date: 2023-03-14T17:15:08Z
#url: https://api.github.com/gists/94b02f6cd84a517760d82b4f1959f763
#owner: https://api.github.com/users/j-a-c-k-maynard

#!/bin/bash -xe
mount -uw /
pushd /
# IMPORTANT: --For Catalina or newer OS-- First reboot apple in restore mode (hold CMD-R when apple logo shows up during reboot)
#            and from menu->util->terminal run 
#            promt> csrutil disable # to disable protection for file name changes
...
do
    if [[ -f ./System/Library/LaunchAgents/${agent}.plist && ! -f ./System/Library/LaunchAgents/${agent}.plist.bak ]] ; then
        mv ./System/Library/LaunchAgents/${agent}.plist ./System/Library/LaunchAgents/${agent}.plist.bak
        echo "[OK] Agent ${agent} disabled"
    else
        ls -l ./System/Library/LaunchAgents/${agent}.plist ./System/Library/LaunchAgents/${agent}.plist.bak || true
        echo "[FAIL] Agent ${agent}.plist didn't exist or has a .bak file already"
    fi
done
...
do
    if [[ -f ./System/Library/LaunchDaemons/${daemon}.plist && ! -f ./System/Library/LaunchDaemons/${daemon}.plist.bak ]] ; then
        mv ./System/Library/LaunchDaemons/${daemon}.plist ./System/Library/LaunchDaemons/${daemon}.plist.bak
        echo "[OK] Daemon ${daemon} disabled"
    else
        ls -l ./System/Library/LaunchDaemons/${daemon}.plist ./System/Library/LaunchDaemons/${daemon}.plist.bak || true
        echo "[FAIL] Daemon ${daemon}.plist didn't exist or has a .bak file already"
    fi
done
mount -ur / || true
echo "DONE"
