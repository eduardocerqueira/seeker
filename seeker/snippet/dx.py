#date: 2025-05-16T17:11:53Z
#url: https://api.github.com/gists/a127e81138b8db0fecf539a2cdd17923
...
org_client = session.client('organizations', config=config)
console = Console()
...
def retry_api_call(func, *args, max_attempts=5, **kwargs):
    """Retry AWS API call with exponential backoff on rate limit errors."""
    for attempt in range(max_attempts):
        try:
            return func(*args, **kwargs)
        except org_client.exceptions.ClientError as e:
            if 'TooManyRequests' in str(e) or 'Throttling' in str(e):
...
def get_nested_ous(parent_id):
    ...
    for page in retry_api_call(paginator.paginate, ParentId=parent_id):
...
def get_accounts_in_ou(ou_id):
    ...
    for page in retry_api_call(paginator.paginate, ParentId=ou_id):
...
def get_policy_details(policy_id):
    ...
    try:
        response = retry_api_call(org_client.describe_policy, PolicyId=policy_id)
...
def get_attached_policies(target_id, target_type):
    ...
    for page in retry_api_call(paginator.paginate, TargetId=target_id, Filter='SERVICE_CONTROL_POLICY'):
...
def process_account(account, root_id, ou_path_map):
    ...
    # Get OU-level SCPs
    ou_scps = []
    current_ou_id = retry_api_call(org_client.list_parents, ChildId=account_id)['Parents'][0]['Id']
    ...
    while current_ou_id != root_id:
        ...
        try:
            current_ou_id = retry_api_call(org_client.list_parents, ChildId=current_ou_id)['Parents'][0]['Id']
        except:
            break
...
def main():
    ...
    response = retry_api_call(org_client.list_roots)
    ...
    for ou in ous:
        ...
        while current_id != root_id:
            parent = retry_api_call(org_client.list_parents, ChildId=current_id)['Parents'][0]
            parent_id = parent['Id']
    ...
    # Get all active accounts (root + all OUs)
    ...
    if not all_accounts:
        console.log("[red]No active accounts found.[/red]")
        return
    ...
    with Progress(
        ...
        # Use 5 workers to balance performance and rate limits
