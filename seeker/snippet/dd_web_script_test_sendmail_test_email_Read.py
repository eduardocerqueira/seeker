...
#阅读已打开的邮件内容,并把发件人、收件人、主题、正文内容，提交到缓存
...
#阅读正文内的信纸
def Read_writingpaper_url(driver):
    try:
        iframe_body=driver.find_element(By.CSS_SELECTOR,".j-mail-content")
        driver.switch_to.frame(iframe_body)
...
def Read_FJ(driver):
    ...
    try:
        ...
        att = driver.find_elements(By.CSS_SELECTOR, ".attach-name")
        for i in att:
            Read_FJ_list.append(i.text)
        return Read_FJ_list
    except:
        return Read_FJ_list
...
def test_email_Read_a(driver,param):
    ...
    param["Read_FJ_list"]=Read_FJ(driver)
