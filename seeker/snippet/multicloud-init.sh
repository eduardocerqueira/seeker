#date: 2023-05-15T17:07:42Z
#url: https://api.github.com/gists/538baec131d263ec77db306850de8c85
#owner: https://api.github.com/users/derhuerst
...
EOF
  ...
  curl -fsSL https://download.docker.com/linux/ubuntu/gpg | apt-key --keyring /etc/apt/trusted.gpg.d/docker.gpg add - > /dev/null 2>&1
  ...
  log "containerd" "installing containerd" ${retval}
}
...
function scaleway_multicloud {
  ...
  if [[ "${tag}" == "null" ]]; then
    echo ",topology.kubernetes.io/region=${NODE_NAME}" >> ${kubelet_dir}/kubelet.env
  else
    echo ",topology.kubernetes.io/region=${tag}" >> ${kubelet_dir}/kubelet.env
  fi
...
EOF
  ...
  # check if apiserver version is between 1.21 and 1.23
  if [[ ${kubernetes_version} =~ ^1\.2[1-3]\.[0-9]*$ ]]; then
  tee /usr/lib/systemd/system/kubelet.service &> /dev/null <<EOF
[Unit]
Description=Kubernetes Kubelet
Documentation=https://github.com/kubernetes/kubernetes
