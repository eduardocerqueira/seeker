#date: 2024-07-18T16:41:29Z
#url: https://api.github.com/gists/62f21dead8e36797256d3b29b8bc40b5
#owner: https://api.github.com/users/FnControlOption
...
preamble = '''// Based on https://github.com/unicode-rs/unicode-properties/blob/master/src/tables.rs
//
...
pub const emoji = struct {""")
    ...
    pub inline fn emojiStatus(c: u21) EmojiStatus {
        // FIXME: do we want to special case ASCII here?
        return util.bsearch_range_value_table(EmojiStatus, c, EMOJI_STATUS).?;
    }

    pub inline fn isEmojiStatusForEmojiCharOrEmojiComponent(s: EmojiStatus) bool {
        return s != .non_emoji;
    }

    pub inline fn isEmojiStatusForEmojiChar(s: EmojiStatus) bool {
        return s != .non_emoji and s != .non_emoji_but_emoji_component;
    }

    pub inline fn isEmojiStatusForEmojiComponent(s: EmojiStatus) bool {
        return switch (s) {
            .emoji_presentation_and_emoji_component,
            .emoji_presentation_and_modifier_and_emoji_component,
            .emoji_other_and_emoji_component,
...
""")
    ...
    emit_table(f, "EMOJI_STATUS", emoji_table, "[]const util.RangeValue(EmojiStatus)", is_pub=False,
...
pub const util = struct {
    pub fn RangeValue(comptime T: type) type {
        return struct { u21, u21, T };
    }

    pub fn bsearch_range_value_table(comptime T: type, c: u21, r: []const RangeValue(T)) ?T {
        const compare = struct {
            fn func(_: void, key: u21, mid_item: RangeValue(T)) std.math.Order {
        ...
        const idx = std.sort.binarySearch(RangeValue(T), c, r, {}, compare.func) orelse
            return null;
        _, _, const cat = r[idx];
        return cat;
    }
