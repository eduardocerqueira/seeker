...
class SmartImport(models.TransientModel):
    ...
    # Supplier context (optioneel voor alleen product updates)
    ...
    def action_import_data(self):
        ...
        try:
            ...
            # Check if we have any supplierinfo fields (non-product fields)
            has_supplierinfo_fields = any(
                not field.startswith('product__') 
                for field in field_mapping.values()
            )
            
            # CRITICAL: Check if supplier is set (required for supplierinfo import)
            if has_supplierinfo_fields and not self.supplier_id:
                raise UserError("Geen leverancier geselecteerd! Selecteer een leverancier om prijzen te kunnen importeren.")
            
            _logger.info(f"Starting import with mapping: {field_mapping}")
            _logger.info(f"Has supplierinfo fields: {has_supplierinfo_fields}")
            ...
            for row_num, row in enumerate(csv_reader, start=1):
                try:
                    ...
                    values = {}
                    product_values = {}  # Separate product fields
                    
                    # Only set partner_id if we have supplierinfo fields and a supplier
                    if has_supplierinfo_fields and self.supplier_id:
                    ...
                    # Decide what to do based on field types
                    if has_supplierinfo_fields:
                        # SUPPLIERINFO IMPORT: Create/update product.supplierinfo
                        ...
                        # PRODUCT-ONLY IMPORT: Update product.product directly
                        if 'product_tmpl_id' in values:
                            product = self.env['product.template'].browse(values['product_tmpl_id'])
                            if product:
                                # Extract only product fields (remove product_tmpl_id itself)
                                update_vals = {k: v for k, v in values.items() if k != 'product_tmpl_id'}
                                if update_vals:
                                    product.write(update_vals)
                                    _logger.info(f"Updated product template for row {row_num}")
                                imported_count += 1
                            else:
                                error_msg = f"Row {row_num}: Product template not found"
                                errors.append(error_msg)
                                error_count += 1
                        else:
                            error_msg = f"Row {row_num}: No product found for mapping. Values: {values}"
    ...
    def _process_row(self, row, mapping, headers):
        ...
        # Only add partner_id if supplier is set
        if self.supplier_id:
